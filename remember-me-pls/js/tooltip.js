...
function buildTooltipContent(d) {
  ...
  content += `
    <strong style="display: block; margin-bottom: 4px;">${d.title || "Untitled"}</strong>
    <em style="display: block; margin-bottom: 2px;">${d.artist || "Unknown Artist"}</em>
    <span style="display: block; margin-bottom: 2px;">${d.sitter || "Unknown Sitter"}</span>
    <span style="display: block;">Year: ${d.year}</span>
  `;
  content += '</div>';
